  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
                  ...
                  {email.isVerified && (
                    <div className="mt-3 flex items-center justify-between border-t border-zinc-900 pt-2.5">
                      <span className="text-xs text-zinc-400">
                        Receive optional security & account alerts
                      </span>
                      <button
                        type="button"
                        role="switch"
                        aria-checked={email.notificationsEnabled}
                        onClick={() => onToggleNotifications(email.id)}
                        className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                          email.notificationsEnabled ? "bg-emerald-600" : "bg-zinc-700"
                        }`}
                      >
                        <span
                          className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out ${
                            email.notificationsEnabled ? "translate-x-4" : "translate-x-0"
                          }`}
                        />
                      </button>
                    </div>
        ...
        <div className="border-t border-zinc-800 bg-zinc-900/50 px-6 py-3.5 flex justify-end">
          <button
            type="button"
            onClick={onClose}
